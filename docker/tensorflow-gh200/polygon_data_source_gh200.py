...
def process_polygon_data_with_cuda_gh200(symbols, data_type="aggregates", timespan="minute"):
    ...
    if data_type == "aggregates":
        ...
        for symbol in symbols:
            ...
            # Calculate technical indicators with XLA compilation
            @tf.function(jit_compile=True)
            def calculate_indicators(prices):
                # Create a TensorFlow-compatible version that doesn't use .numpy()
                # Use tf.py_function to wrap the CuPy-based function
                def tf_calculate_indicators(prices_tensor):
                    # This runs outside of XLA context
                    return tf.py_function(
                        lambda x: calculate_technical_indicators(x),
                        [prices_tensor],
                        [tf.float32, tf.float32, tf.float32, tf.float32]
                    )

                ema, rsi, upper_band, lower_band = tf_calculate_indicators(
                    prices)
                return ema, rsi, upper_band, lower_band

            start_time = time.time()
            ema, rsi, upper_band, lower_band = calculate_indicators(prices)
            indicators_time = time.time() - start_time

            # Calculate MACD with XLA compilation
            @tf.function(jit_compile=True)
            def calculate_macd(prices):
                # Use the fixed fused_financial_ops function directly
                # This should work because we fixed the dimension mismatch issue
