...
CACHE_TTL = int(os.environ.get('UNUSUAL_WHALES_CACHE_TTL', 300))  # 5 minutes default TTL
MAX_RETRIES = int(os.environ.get('UNUSUAL_WHALES_MAX_RETRIES', 3))
RETRY_BACKOFF_FACTOR = float(os.environ.get('UNUSUAL_WHALES_RETRY_BACKOFF_FACTOR', 0.5))
CONNECTION_TIMEOUT = int(os.environ.get('UNUSUAL_WHALES_CONNECTION_TIMEOUT', 15))
...
class AsyncConnectionPool:
    ...
    def __init__(self, max_retries=MAX_RETRIES, backoff_factor=RETRY_BACKOFF_FACTOR,
        ...
        logger.info(f"Async connection pool initialized with max_pool_size={max_pool_size}, max_retries={max_retries}")
            ...
            try:
                async with self.session.get(url, params=params, headers=headers) as response:
                    ...
                    if response.status == 429:
                        ...
                        logger.warning(f"Rate limit hit, retrying in {wait_time:.2f}s (attempt {retry+1}/{self.max_retries})")
                    ...
                    wait_time = (2 ** retry) * self.backoff_factor + random.uniform(0, 1)
                    logger.warning(f"Server error, retrying in {wait_time:.2f}s (attempt {retry+1}/{self.max_retries})")
                    await asyncio.sleep(wait_time)
                ...
                wait_time = (2 ** retry) * self.backoff_factor + random.uniform(0, 1)
                logger.warning(f"Connection error, retrying in {wait_time:.2f}s (attempt {retry+1}/{self.max_retries}): {e}")
                await asyncio.sleep(wait_time)
...
class GPUUnusualWhalesClient:
    ...
    def __init__(self, api_key=UNUSUAL_WHALES_API_KEY, redis_client=None, use_gpu=USE_GPU,
        ...
        if not self.api_key:
            ...
            logger.info(f"Initialized Unusual Whales API client with API key: {self.api_key[:4]}****{self.api_key[-4:] if len(self.api_key) > 8 else ''}")
    ...
    def _initialize_gpu(self):
        ...
        try:
            ...
            if device_count > 0:
                ...
                logger.error("GPU is required for this application")
                sys.exit(1)
        except ImportError:
            logger.error("CuPy not available. GPU acceleration is required")
            logger.error("Please install CuPy with: pip install cupy-cuda12x")
            sys.exit(1)
        except Exception as e:
            logger.error(f"Fatal error initializing GPU: {e}")
            sys.exit(1)
    ...
    async def get_flow_alerts(self, ticker, is_ask_side=True, is_bid_side=True, limit=100):
        ...
        try:
            ...
            if data and "data" in data:
                logger.info(f"Flow alerts retrieved for {ticker}: {len(data['data'])} items")
                ...
                if self.redis_client:
                    ...
                    for alert in data["data"]:
                        ...
                        pipeline.zadd(f"unusual_whales:flow:{ticker}", {json.dumps(alert): score})
                    
                    # Store last update timestamp
                    pipeline.set(f"unusual_whales:flow:{ticker}:last_update", datetime.now().isoformat())
                ...
                if self.use_gpu and self.gpu_initialized:
                    self.thread_pool.submit(self._process_flow_alerts_gpu, ticker, data["data"])
                
                return data["data"]
            else:
                error_msg = data.get("error", "Unknown error") if isinstance(data, dict) else "Unknown error"
                logger.warning(f"Failed to get flow alerts for {ticker}: {error_msg}")
                return []
        except Exception as e:
    ...
    def _process_flow_alerts_gpu(self, ticker, alerts):
        ...
        try:
            ...
            premiums = [float(alert.get("total_premium", 0)) for alert in alerts]
            ...
            # Count call vs put
            call_count = sum(1 for alert in alerts if alert.get("type") == "call")
            put_count = sum(1 for alert in alerts if alert.get("type") == "put")
            ...
            logger.error(f"Error processing flow alerts for {ticker} on GPU: {e}")
            ...
            if data and "data" in data:
                ...
                if self.redis_client:
                    ...
                    for alert in data["data"]:
                        ...
                        if tape_time:
                            try:
                                dt = datetime.fromisoformat(tape_time.replace("Z", "+00:00"))
                                score = dt.timestamp()
                            except ValueError:
                                score = time.time()
                        else:
                        ...
                        pipeline.zadd("unusual_whales:alerts", {json.dumps(alert): score})
                    
                    # Store last update timestamp
                    pipeline.set("unusual_whales:alerts:last_update", datetime.now().isoformat())
    ...
    async def get_alert_configurations(self):
        ...
        try:
            ...
            if data and "data" in data:
                logger.info(f"Alert configurations retrieved: {len(data['data'])} items")
                ...
                if self.redis_client:
                    ...
                    for config in data["data"]:
                        ...
                        if config_id:
                            pipeline.hset("unusual_whales:alert_configs", config_id, json.dumps(config))
                    
                    # Store last update timestamp
                    pipeline.set("unusual_whales:alert_configs:last_update", datetime.now().isoformat())
    ...
    async def get_recent_dark_pool_trades(self, date=None, limit=100, max_premium=None, 
        ...
        try:
            ...
            if data and "data" in data:
                logger.info(f"Recent dark pool trades retrieved: {len(data['data'])} items")
                ...
                if self.redis_client:
                    ...
                    for trade in data["data"]:
                        ...
                        pipeline.zadd("unusual_whales:darkpool:recent", {json.dumps(trade): score})
                    
                    # Store last update timestamp
                    pipeline.set("unusual_whales:darkpool:recent:last_update", datetime.now().isoformat())
                ...
                # Process with GPU if available
                if self.use_gpu and self.gpu_initialized:
                    self.thread_pool.submit(self._process_dark_pool_trades_gpu, data["data"])
                
                return data["data"]
            else:
                error_msg = data.get("error", "Unknown error") if isinstance(data, dict) else "Unknown error"
                logger.warning(f"Failed to get recent dark pool trades: {error_msg}")
                return []
        except Exception as e:
    ...
    def _process_dark_pool_trades_gpu(self, trades):
        ...
        try:
            ...
            for ticker, ticker_trades in trades_by_ticker.items():
                # Extract relevant data
                premiums = [float(trade.get("premium", 0)) for trade in ticker_trades]
                sizes = [int(trade.get("size", 0)) for trade in ticker_trades]
                prices = [float(trade.get("price", 0)) for trade in ticker_trades]
    ...
    async def get_dark_pool_trades(self, ticker, date=None, limit=500, max_premium=None, 
        ...
        try:
            ...
            if data and "data" in data:
                logger.info(f"Dark pool trades retrieved for {ticker}: {len(data['data'])} items")
                ...
                if self.redis_client:
                    ...
                    for trade in data["data"]:
                        ...
                        pipeline.zadd(f"unusual_whales:darkpool:{ticker}", {json.dumps(trade): score})
                    
                    # Store last update timestamp
                    pipeline.set(f"unusual_whales:darkpool:{ticker}:last_update", datetime.now().isoformat())
                ...
                error_msg = data.get("error", "Unknown error") if isinstance(data, dict) else "Unknown error"
                logger.warning(f"Failed to get dark pool trades for {ticker}: {error_msg}")
                return []
        except Exception as e:
    ...
    async def start_scheduled_tasks(self, event_loop=None):
        ...
        self.scheduled_tasks['unusual_activity'] = self.event_loop.create_task(
            self._schedule_task(self.get_alerts, REFRESH_INTERVALS['unusual_activity'])
        )
        
        # Schedule dark pool updates
        self.scheduled_tasks['dark_pool'] = self.event_loop.create_task(
            self._schedule_task(self.get_recent_dark_pool_trades, REFRESH_INTERVALS['dark_pool'])
        )
        ...
        while self.running:
            for ticker in tickers:
                ...
                try:
                    ...
                    logger.error(f"Error updating flow alerts for {ticker}: {e}")
        ...
        while self.running:
            for ticker in tickers:
                ...
                try:
                    ...
                    logger.error(f"Error updating dark pool trades for {ticker}: {e}")
...
async def main():
    ...
    ticker = "AAPL"
    flow_alerts = await client.get_flow_alerts(ticker)
    print(f"\nFlow Alerts for {ticker}:")
    print(f"Retrieved {len(flow_alerts)} alerts")
    
    # Get recent dark pool trades
    dark_pool_trades = await client.get_recent_dark_pool_trades(limit=10)
    print("\nRecent Dark Pool Trades:")
    print(f"Retrieved {len(dark_pool_trades)} trades")
