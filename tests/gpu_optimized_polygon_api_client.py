...
class GPUPolygonAPIClient:
        ...
        try:
            ...
            if data and "status" in data and data["status"] == "OK":
                ...
                if self.redis_client:
                    ...
                    self.redis_client.set("market:status:last_update", datetime.now().isoformat())
        ...
        try:
            ...
            if data and "status" in data and data["status"] == "OK":
                ...
                if self.redis_client:
                    ...
                    if result:
                        self.redis_client.hset(f"stock:{ticker}:metadata", mapping=result)
                        self.redis_client.set(f"stock:{ticker}:metadata:last_update", datetime.now().isoformat())
    ...
    async def get_aggregates(self, ticker, multiplier=1, timespan="minute",
        ...
        cache_key = [f"aggregates:{ticker}:{multiplier}:{timespan}:{from_date}:{to_date}:{adjusted}"]
        ...
        if data.get("status") == "OK" and "results" in data and data["results"]:
            ...
            if self.redis_client:
                ...
                if candles_dict:
                    ...
                    pipeline.set(f"{redis_key}:last_update", datetime.now().isoformat())
                    pipeline.execute()
    ...
    async def process_data_with_gpu(self, data):
        ...
        if not self.use_gpu or not self.gpu_initialized:
            logger.warning("GPU processing requested but GPU is not available or initialized")
