...
class MonitoringSystem:
    ...
    def start(self):
        ...
        self.threads.append(threading.Thread(target=self._system_metrics_worker, daemon=True))
        self.threads.append(threading.Thread(target=self._data_metrics_worker, daemon=True))
        self.threads.append(threading.Thread(target=self._trading_metrics_worker, daemon=True))
        self.threads.append(threading.Thread(target=self._alert_worker, daemon=True))
        ...
        logger.info(f"Monitoring system started on port {self.config['metrics_port']}")
    ...
    def _setup_metrics(self):
        ...
        self.metrics = {
            # System metrics
            'system_status': Gauge('system_status', 'Trading system status (1=running, 0=stopped)'),
            ...
            'signals_generated': Counter('signals_generated_total', 'Total signals generated', ['source']),
            'trades_executed': Counter('trades_executed_total', 'Total trades executed', ['direction']),
            'pnl_total': Gauge('pnl_total', 'Total P&L in dollars'),
            'pnl_daily': Gauge('pnl_daily', 'Daily P&L in dollars'),
            'win_rate': Gauge('win_rate', 'Win rate percentage'),
            'average_profit': Gauge('average_profit', 'Average profit per trade in dollars'),
            'average_loss': Gauge('average_loss', 'Average loss per trade in dollars'),
            ...
            # Model metrics
            'model_prediction_count': Counter('model_prediction_count', 'Number of model predictions', ['model_name']),
            'model_inference_time': Summary('model_inference_time', 'Model inference time in ms', ['model_name']),
            'signal_quality': Gauge('signal_quality', 'Signal quality score', ['model_name'])
        }
    ...
    def _collect_redis_metrics(self):
        ...
        try:
            ...
            prefixes = ['stock:', 'options:', 'darkpool:', 'watchlist:', 'positions:', 'orders:', 'signals:']
    ...
    def _check_data_freshness(self):
        ...
        try:
            ...
            for ticker in self._get_active_tickers():
                last_update = self.redis.hget(f"stock:{ticker}:last_trade", "timestamp")
                
                if last_update:
                    last_update = float(last_update) / 1000.0  # Convert ms to seconds
                    freshness = time.time() - last_update
                    self.metrics['data_freshness'].labels(data_type=f"stock:{ticker}").set(freshness)
            ...
            for data_type in market_data_types:
                ...
                if market_data:
                    ...
                    freshness = time.time() - last_update
                    self.metrics['data_freshness'].labels(data_type=f"market:{data_type}").set(freshness)
                    
                    # Alert if market data is stale
                    if freshness > self.config['alert_thresholds']['data_freshness'] * 2:  # More lenient for market data
                        self._trigger_alert(
        ...
        try:
            ...
            for key, value in api_metrics.items():
                if b':' in key:
                    ...
                    self.metrics['api_requests'].labels(api=api, endpoint=endpoint).inc(metrics.get('requests', 0))
                    self.metrics['api_errors'].labels(api=api, endpoint=endpoint).inc(metrics.get('errors', 0))
                    
                    # Calculate error rate
                    if metrics.get('requests', 0) > 0:
                        error_rate = metrics.get('errors', 0) / metrics.get('requests', 0)
        ...
        try:
            ...
            if daily_stats:
                ...
                if 'max_drawdown' in stats:
                    self.metrics['max_drawdown'].set(float(stats['max_drawdown']) * 100)
                    
                if 'current_exposure' in stats:
                    self.metrics['exposure'].set(float(stats['current_exposure']))
            ...
            if trades:
                ...
                long_trades = sum(1 for _, t in trades.items() if json.loads(t).get('direction') == 'long')
                short_trades = sum(1 for _, t in trades.items() if json.loads(t).get('direction') == 'short')
                
                self.metrics['trades_executed'].labels(direction='long').inc(long_trades)
                self.metrics['trades_executed'].labels(direction='short').inc(short_trades)
        ...
        try:
            ...
            if queue_metrics:
                for queue_name, metrics_data in queue_metrics.items():
                    ...
                    # Update Prometheus metrics
                    self.metrics['queue_depth'].labels(queue_name=queue_name).set(metrics.get('depth', 0))
                    
                    # Record latency observations
                    latencies = metrics.get('latencies', [])
                    for latency in latencies:
                        self.metrics['queue_latency'].labels(queue_name=queue_name).observe(latency)
    ...
    def _get_active_tickers(self):
        ...
        try:
            ...
            # Convert to strings
            watchlist = [ticker.decode('utf-8') if isinstance(ticker, bytes) else ticker for ticker in watchlist]
