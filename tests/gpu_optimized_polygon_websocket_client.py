...
MAX_RECONNECT_ATTEMPTS = int(os.environ.get('POLYGON_MAX_RECONNECT_ATTEMPTS', 10))
RECONNECT_DELAY = float(os.environ.get('POLYGON_RECONNECT_DELAY', 1.0))
REDIS_HOST = os.environ.get('REDIS_HOST', 'localhost')
REDIS_PORT = int(os.environ.get('REDIS_PORT', 6380))
REDIS_DB = int(os.environ.get('REDIS_DB', 0))
USE_GPU = os.environ.get('USE_GPU', 'true').lower() == 'true'
BUFFER_SIZE = int(os.environ.get('BUFFER_SIZE', 1000))  # Number of messages to buffer before batch processing
...
class GPUPolygonWebSocketClient:
    ...
    def __init__(self, api_key=POLYGON_API_KEY, redis_client=None, use_gpu=USE_GPU,
        ...
        if not self.api_key:
            ...
            logger.info(f"Initialized Polygon WebSocket client with API key: {self.api_key[:4]}****{self.api_key[-4:] if len(self.api_key) > 8 else ''}")
    ...
    async def _reconnect(self):
        ...
        logger.info(f"Attempting to reconnect (attempt {self.reconnect_count}/{self.max_reconnect_attempts}) in {delay:.2f} seconds...")
        ...
        if 'status' in message:
            ...
            if status == 'connected':
                ...
                if message.get('message') == 'authenticated':
                    ...
                    if action == 'subscribe':
                        logger.info(f"Successfully subscribed to: {message.get('params', '')}")
                    elif action == 'unsubscribe':
                        logger.info(f"Successfully unsubscribed from: {message.get('params', '')}")
                        
            elif status == 'error':
                logger.error(f"Error from Polygon WebSocket: {message.get('message', '')}")
        ...
        with self.buffer_lock:
            if event_type == 'T':  # Trade
                ...
                if len(self.trade_buffer) >= self.buffer_size:
                    ...
                    self.trade_buffer = []
                    self.thread_pool.submit(self._process_trade_batch, trades_to_process)
                ...
                if len(self.quote_buffer) >= self.buffer_size:
                    ...
                    self.quote_buffer = []
                    self.thread_pool.submit(self._process_quote_batch, quotes_to_process)
                ...
                if len(self.agg_buffer) >= self.buffer_size:
                    ...
                    self.agg_buffer = []
                    self.thread_pool.submit(self._process_agg_batch, aggs_to_process)
    ...
    def _process_quote_batch(self, quotes):
        ...
        try:
            ...
            for ticker, ticker_quotes in quotes_by_ticker.items():
                ...
                latest_mid = (latest_bid + latest_ask) / 2 if latest_bid > 0 and latest_ask > 0 else 0
    ...
    def _process_agg_batch(self, aggs):
        ...
        try:
            ...
            for ticker, ticker_aggs in aggs_by_ticker.items():
                ...
                timestamps = [a.get('s', 0) for a in ticker_aggs]  # Start timestamps
                ...
                if self.redis_client:
                    ...
                    timespan = "minute" if latest_agg.get('ev') == 'AM' else "second"
                    ...
                    for agg in ticker_aggs:
                        ...
                        self.redis_client.hset(candle_key, timestamp, json.dumps(candle_data))
    ...
    def start(self):
        ...
        def run_event_loop():
            ...
            try:
                ...
                self.heartbeat_task = self.event_loop.create_task(self._heartbeat())
    ...
    def subscribe(self, channels):
        ...
        if self.connected and self.event_loop and not self.event_loop.is_closed():
            ...
            async def send_subscribe():
                try:
                    await self.ws.send(json.dumps(subscribe_message))
                    logger.info(f"Subscribed to channels: {', '.join(new_channels)}")
                except Exception as e:
                    logger.error(f"Error subscribing to channels: {e}")
                    
            asyncio.run_coroutine_threadsafe(send_subscribe(), self.event_loop)
        else:
            logger.info(f"Added channels to subscription list (will subscribe when connected): {', '.join(new_channels)}")
        ...
        if self.connected and self.event_loop and not self.event_loop.is_closed():
            ...
            async def send_unsubscribe():
                try:
                    ...
                    logger.info(f"Unsubscribed from channels: {', '.join(removed_channels)}")
                except Exception as e:
                    logger.error(f"Error unsubscribing from channels: {e}")
                    
            asyncio.run_coroutine_threadsafe(send_unsubscribe(), self.event_loop)
        else:
            logger.info(f"Removed channels from subscription list: {', '.join(removed_channels)}")
    ...
    def remove_message_handler(self, event_type, handler):
        ...
        if event_type in self.message_handlers:
            if handler in self.message_handlers[event_type]:
                ...
                logger.info(f"Removed message handler for event type: {event_type}")
...
if __name__ == "__main__":
    # Create client
    client = GPUPolygonWebSocketClient()
    ...
    def quote_handler(message):
        ...
        if ticker and bid_price and ask_price and timestamp:
            dt = datetime.fromtimestamp(timestamp / 1000.0)
            print(f"[{dt}] QUOTE: {ticker} - Bid: ${bid_price:.2f} x {bid_size}, Ask: ${ask_price:.2f} x {ask_size}")
