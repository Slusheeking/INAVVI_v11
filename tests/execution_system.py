...
class ExecutionSystem:
    ...
    def start(self):
        ...
        self.threads.append(threading.Thread(target=self._signal_listener_worker, daemon=True))
        self.threads.append(threading.Thread(target=self._execution_worker, daemon=True))
        self.threads.append(threading.Thread(target=self._position_monitor_worker, daemon=True))
        self.threads.append(threading.Thread(target=self._order_status_worker, daemon=True))
        self.threads.append(threading.Thread(target=self._market_data_worker, daemon=True))
    ...
    def _initialize_state(self):
        ...
        try:
            ...
            if positions:
                logger.warning(f"Found {len(positions)} existing positions, closing them")
            ...
            logger.error(f"Error initializing execution system state: {str(e)}", exc_info=True)
                ...
                if message and message['type'] == 'message':
                    ...
                    try:
                        ...
                        if self._validate_signal(signal):
                            ...
                            logger.info(f"Received valid signal for {signal['ticker']}")
                        else:
        ...
        while self.running:
            try:
                ...
                if not self._check_market_status(signal):
                    logger.info(f"Market closed, skipping signal for {signal['ticker']}")
                    self.signal_queue.task_done()
                    continue
                
                # Check if we can take new positions
                if not self._check_position_limits(signal):
                    logger.info(f"Position limits reached, skipping signal for {signal['ticker']}")
                    self.signal_queue.task_done()
                    continue
    ...
    def _market_data_worker(self):
        ...
        while self.running:
            try:
                ...
                # Store in Redis
                self.redis.set("market:is_open", "1" if is_market_open else "0")
    ...
    def _is_near_market_close(self):
        ...
        try:
            ...
            now = datetime.datetime.now(pytz.timezone('US/Eastern'))
            close_time = clock.next_close.astimezone(pytz.timezone('US/Eastern'))
    ...
    def _execute_signal(self, signal):
        ...
        try:
            ...
            params = self._calculate_order_parameters(signal, current_price, position_size)
            ...
            # Store in Redis
            self.redis.hset(f"orders:pending", order_id, json.dumps(self.pending_orders[order_id]))
            
            logger.info(f"Submitted order {order_id} for {ticker}: {position_size} shares at {params['limit_price']}")
            
        except Exception as e:
            logger.error(f"Error executing signal for {signal['ticker']}: {str(e)}")
    ...
    def _calculate_position_size(self, signal, current_price):
        ...
        try:
            ...
            if 'position_size' in signal and signal['position_size'] > 0:
                ...
                stop_loss = signal.get('stop_loss', current_price * (1 - self.config['stop_loss_default']))
                risk_amount = account['equity'] * self.config['max_loss_per_trade']
    ...
    def _calculate_order_parameters(self, signal, current_price, position_size):
        ...
        if order_type == 'limit':
            ...
            if signal['direction'] == 'long':
                limit_price = current_price * (1 + self.config['limit_price_offset'])
            else:
                limit_price = current_price * (1 - self.config['limit_price_offset'])
        else:
        ...
        if 'price_target' in signal and signal['price_target'] > 0:
            ...
            if signal['direction'] == 'long':
                take_profit = current_price * (1 + self.config['take_profit_default'])
            else:
                take_profit = current_price * (1 - self.config['take_profit_default'])
    ...
    def _handle_filled_order(self, order_id, order_info):
        ...
        try:
            ...
            # Store in Redis
            self.redis.hset("positions:active", f"{ticker}:{order_id}", json.dumps(position))
            ...
            logger.info(f"Order filled for {ticker}: {filled_qty} shares at {filled_price}")
    ...
    def _handle_canceled_order(self, order_id, order_info):
        ...
        try:
            ...
            logger.info(f"Order canceled for {order_info['ticker']}: {order_id}")
    ...
    def _handle_rejected_order(self, order_id, order_info):
        ...
        try:
            ...
            logger.warning(f"Order rejected for {order_info['ticker']}: {order_id}")
    ...
    def _update_position_stats(self, ticker, current_price):
        ...
        try:
            ...
            logger.error(f"Error updating position stats for {ticker}: {str(e)}")
    ...
    def _check_exit_conditions(self, ticker, current_price):
        ...
        try:
            ...
            if position_age >= self.config['position_timeout']:
                logger.info(f"Time-based exit triggered for {ticker} after {position_age/3600:.1f} hours")
                return True
            ...
            logger.error(f"Error checking exit conditions for {ticker}: {str(e)}")
            return False
    ...
    def _exit_position(self, ticker, reason, price=None):
        ...
        try:
            ...
            if position['direction'] == 'long':
                ...
                realized_pnl_pct = (price / position['entry_price'] - 1) * 100 if price else 0
            else:
                realized_pnl = entry_value - exit_value
                realized_pnl_pct = (position['entry_price'] / price - 1) * 100 if price else 0
            ...
            self.redis.hdel("positions:active", f"{ticker}:{position['order_id']}")
            
            # Update Redis stats
            self.redis.hmset("execution:daily_stats", self.daily_stats)
            
            logger.info(f"Exited position for {ticker}: {quantity} shares at ~{price}, PnL: ${realized_pnl:.2f} ({realized_pnl_pct:.2f}%)")
    ...
    def _submit_trailing_stop(self, ticker, position):
        ...
        try:
            ...
            logger.info(f"Submitted trailing stop for {ticker}: {trail_percent:.1f}% trail")
            
            return True
            
        except Exception as e:
            logger.error(f"Error submitting trailing stop for {ticker}: {str(e)}")
            return False
    ...
    def _cancel_position_orders(self, ticker):
        ...
        try:
            ...
            logger.error(f"Error canceling position orders for {ticker}: {str(e)}")
            return False
    ...
    def _update_daily_stats(self):
        ...
        try:
            ...
            if equity:
                drawdown = (self.daily_stats.get('peak_equity', equity) - equity) / self.daily_stats.get('peak_equity', equity)
                self.daily_stats['max_drawdown'] = max(self.daily_stats.get('max_drawdown', 0), drawdown)
