...
class ReportingSystem:
    ...
    def __init__(self, redis_client):
        ...
        self.config = {
            'report_schedule': {
                ...
                'position_updates': ['09:35', '12:00', '15:55'],  # 9:35 AM, 12:00 PM, 3:55 PM
        ...
        try:
            ...
            if trades:
                ...
                for i, trade in enumerate(trades[:5]):  # Show top 5 trades
                    direction = ":arrow_up:" if trade.get('direction') == 'long' else ":arrow_down:"
    ...
    def send_positions_report(self):
        ...
        try:
            ...
            for key, pos_data in positions.items():
                position = json.loads(pos_data.decode('utf-8') if isinstance(pos_data, bytes) else pos_data)
                position_list.append(position)
                total_value += position.get('current_value', 0)
                total_pnl += position.get('unrealized_pnl', 0)
            
            # Sort positions by unrealized P&L
            position_list.sort(key=lambda x: x.get('unrealized_pnl', 0), reverse=True)
    ...
    def send_portfolio_report(self):
        ...
        try:
            ...
            if not account_info:
                logger.warning("No account information available for portfolio report")
                return
            ...
            if equity_history:
                ...
                if len(equity_history) >= 1:
                    daily_change = (current_equity / equity_history[0] - 1) * 100
                if len(equity_history) >= 5:
                    weekly_change = (current_equity / equity_history[4] - 1) * 100
                if len(equity_history) >= 20:
                    monthly_change = (current_equity / equity_history[19] - 1) * 100
    ...
    def _get_todays_trades(self):
        ...
        try:
            ...
            for trade_id, trade_data in trades.items():
                trade = json.loads(trade_data.decode('utf-8') if isinstance(trade_data, bytes) else trade_data)
                
                # Check if trade was executed today
                trade_time = datetime.datetime.fromtimestamp(trade.get('exit_time', 0)).date()
                if trade_time == today:
                    trade_list.append(trade)
            
            # Sort by P&L
            trade_list.sort(key=lambda x: x.get('realized_pnl', 0), reverse=True)
    ...
    def _calculate_weekly_stats(self):
        ...
        try:
            ...
            profitable_trades = sum(1 for t in trade_list if t.get('realized_pnl', 0) > 0)
            ...
            for trade in trade_list:
                day = datetime.datetime.fromtimestamp(trade.get('exit_time', 0)).strftime('%Y-%m-%d')
            ...
            for ticker, ticker_trades in trades_by_ticker.items():
                pnl_by_ticker[ticker] = sum(t.get('realized_pnl', 0) for t in ticker_trades)
    ...
    def _calculate_monthly_stats(self):
        ...
        try:
            ...
            for strategy, strategy_trades in trades_by_strategy.items():
                ...
                strategy_win_rate = (sum(1 for t in strategy_trades if t.get('realized_pnl', 0) > 0) / len(strategy_trades) * 100) if strategy_trades else 0
            ...
            winning_trades = [t for t in trade_list if t.get('realized_pnl', 0) > 0]
            losing_trades = [t for t in trade_list if t.get('realized_pnl', 0) < 0]
            
            avg_win = sum(t.get('realized_pnl', 0) for t in winning_trades) / len(winning_trades) if winning_trades else 0
            avg_loss = sum(abs(t.get('realized_pnl', 0)) for t in losing_trades) / len(losing_trades) if losing_trades else 0
            
            avg_win_loss_ratio = avg_win / avg_loss if avg_loss > 0 else 0
            profit_factor = sum(t.get('realized_pnl', 0) for t in winning_trades) / sum(abs(t.get('realized_pnl', 0)) for t in losing_trades) if losing_trades else 0
    ...
    def _is_market_open(self):
        ...
        try:
            ...
            eastern = now.astimezone(datetime.timezone(-datetime.timedelta(hours=5)))  # Eastern Time
