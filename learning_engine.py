...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(os.environ.get("LOGS_DIR", "./logs"),
                         "learning_engine.log"),
        ),
...
if PROMETHEUS_AVAILABLE:
    ...
    MODEL_VERSIONS = prom.Gauge(
        "learning_engine_model_versions", "Number of model versions", [
            "model_name"],
    )
...
class ModelPerformanceTracker:
    ...
    def log_metrics(self, model_name: str, version: int, metrics: dict[str, float]) -> None:
        ...
        if self.redis:
            try:
                ...
                logger.exception(f"Error storing metrics in Redis: {e!s}")
    ...
    def set_baseline(self, model_name: str, metrics: dict[str, float]) -> None:
        ...
        if self.redis:
            try:
                ...
                logger.exception(f"Error storing baseline in Redis: {e!s}")

    def compare_to_baseline(self, model_name: str, version: int) -> dict[str, float]:
        """Compare metrics to baseline and return differences"""
        if (
            model_name not in self.metrics_history
            or version not in self.metrics_history[model_name]
        ):
        ...
        for key in baseline:
            if (
                key in current and isinstance(baseline[key], int | float) and key not in {
                    "timestamp", "version"}
            ):
    ...
    def get_best_version(
        ...
        valid_versions = {
            v: metrics for v, metrics in versions.items() if metric in metrics
        }
    ...
    def _prepare_for_redis(self, metrics: dict) -> dict:
        ...
        for k, v in metrics.items():
            ...
            if isinstance(
                v, np.int_ | np.intc | np.intp | np.int8 | np.int16 | np.int32 | np.int64,
            ):
                result[k] = int(v)
...
class ModelRegistry:
    ...
    def __init__(self, models_dir: str, redis_client=None) -> None:
    ...
    def _load_metadata(self) -> None:
        ...
        if os.path.exists(metadata_path):
            try:
                ...
                logger.exception(f"Error loading model metadata: {e!s}")
                self.models_metadata = {}
    ...
    def _save_metadata(self) -> None:
        ...
        try:
            ...
            logger.exception(f"Error saving model metadata: {e!s}")
    ...
    def register_model(
        ...
        try:
            ...
            self.models_metadata[model_name]["versions"][
                str(new_version)
            ] = version_metadata
            ...
            if PROMETHEUS_AVAILABLE:
                try:
                    ...
                    elif model_type == "sklearn" and hasattr(
            ...
            logger.exception(f"Error registering model {model_name}: {e!s}")
            raise

    def load_model(self, model_name: str, version: int | None = None) -> tuple[Any, dict]:
        """
        ...
        if version_str not in self.models_metadata[model_name]["versions"]:
            msg = f"Version {version} of model {model_name} not found"
            raise ValueError(msg)
        ...
        try:
            ...
            logger.exception(
                f"Error loading model {model_name} v{version}: {e!s}")
            raise
    ...
    def get_model_versions(self, model_name: str) -> list[int]:
        ...
        return [int(v) for v in self.models_metadata[model_name]["versions"]]
    ...
    def delete_model_version(self, model_name: str, version: int) -> bool:
        ...
        if os.path.exists(model_path):
            try:
                ...
                logger.exception(
                    f"Error deleting model file {model_path}: {e!s}")
                return False
        ...
        if self.models_metadata[model_name]["latest_version"] == version:
            if self.models_metadata[model_name]["versions"]:
                self.models_metadata[model_name]["latest_version"] = max(
                    int(v) for v in self.models_metadata[model_name]["versions"]
                )
            else:
        ...
        if self.redis:
            try:
                ...
                logger.exception(
                    f"Error updating Redis after model deletion: {e!s}")

        logger.info(f"Deleted {model_name} v{version} from registry")
        return True

    def _get_model_filename(
        self, model_name: str, version: int, model_type: str,
    ) -> str:
    ...
    def _update_redis_metadata(self, model_name: str, version: int, metadata: dict) -> None:
        ...
        try:
            ...
            # Store metadata
            self.redis.hset(
                f"model:registry:{model_name}", str(
                    version), json.dumps(metadata),
            )
            ...
            logger.exception(f"Error updating Redis model metadata: {e!s}")
...
class ModelDeployer:
    ...
    def __init__(self, model_registry: ModelRegistry, redis_client=None) -> None:
    ...
    def _load_deployed_models(self) -> None:
        ...
        if self.redis:
            try:
                ...
                logger.exception(
                    f"Error loading deployed models from Redis: {e!s}")

    def deploy_model(
        self, model_name: str, version: int, environment: str = "production",
    ) -> bool:
        """
        ...
        try:
            ...
            if self.redis:
                try:
                    ...
                    logger.exception(
                        f"Error updating Redis for model deployment: {e!s}")
            ...
            if PROMETHEUS_AVAILABLE:
                try:
                    ...
                    MODEL_TRAINING_TIME.labels(
                        model_name=model_name,
                        model_type=self.registry.models_metadata[model_name][
                            "model_type"
                        ],
            ...
            # Send notification to frontend about model deployment
            if self.redis:
                try:
                    # Get model metrics if available
                    metrics_key = f"model:metrics:{model_name}:{version}"
                    metrics = self.redis.hgetall(metrics_key)

                    # Create notification for frontend
                    notification = {
                        "type": "model_deployment",
                        "message": f"Model {model_name} v{version} deployed to {environment}",
                        "level": "success",
                        "timestamp": time.time(),
                        "details": {
                            "model_name": model_name,
                    ...
                    # Also store in model_deployment category
                    self.redis.lpush("frontend:model_deployment",
                                     json.dumps(notification))
                    self.redis.ltrim("frontend:model_deployment", 0, 49)

                    logger.info(
                        f"Model deployment notification sent to frontend for {model_name} v{version}")
                except Exception as e:
                    logger.error(
                        f"Error sending model deployment notification: {e}")

            return True

        except Exception as e:
            logger.exception(
                f"Error deploying model {model_name} v{version}: {e!s}")
            return False

    def rollback_model(self, model_name: str, to_version: int | None = None) -> bool:
        """
        ...
        try:
            ...
            logger.exception(f"Error rolling back model {model_name}: {e!s}")
            return False
    ...
    def get_deployed_model(self, model_name: str) -> tuple[Any, dict]:
        ...
        try:
            ...
            logger.exception(
                f"Error loading deployed model {model_name} v{version}: {e!s}",
            )
            return None, None
...
class DriftDetector:
    ...
    def store_reference_distribution(self, name: str, data, metadata: dict | None = None) -> None:
        """
        ...
        try:
            if isinstance(data, pd.DataFrame):
                ...
                if self.redis:
                    try:
                        ...
                        logger.exception(
                            f"Error storing reference distribution in Redis: {e!s}",
                        )
            ...
            logger.exception(f"Error storing reference distribution: {e!s}")
        ...
        if name not in self.reference_distributions:
            ...
            if self.redis:
                try:
                    ...
                    logger.exception(
                        f"Error loading reference distribution from Redis: {e!s}",
                    )
        ...
        try:
            ...
            if not isinstance(current_data, pd.DataFrame):
                return {
                    ...
                    "error": "Current data must be a DataFrame",
                }
            ...
            for column, ref_stats in reference["stats"].items():
                if column in current_data.columns:
                    ...
                    if len(column_data) > 0 and np.issubdtype(
                        ...
                        try:
                            ...
                            # Check for drift
                            feature_drift = ks_stat > self.drift_thresholds[
                                "ks_threshold"
                            ] and (mean_pct_change > 0.2 or std_pct_change > 0.3)
                            ...
                            logger.exception(
                                f"Error calculating drift for {column}: {e!s}",
                            )
            ...
            if self.redis:
                try:
                    ...
                    self.redis.expire(
                        f"drift:details:{name}:{int(time.time())}", 60 *
                        60 * 24 * 7,
                    )
                    ...
                    if drift_detected:
                        try:
                            ...
                            notification = {
                                ...
                                "message": f"Data drift detected in {name} distribution",
                                "level": "warning",
                                "timestamp": time.time(),
                                "details": {
                                    "distribution_name": name,
                                    "drifted_features": sum(1 for v in drift_metrics.values() if isinstance(v, dict) and v.get("drift_detected", False)),
                                    ...
                                    ][:5]  # Top 5 most affected features
                                }
                            }
                            ...
                            logger.warning(
                                f"Drift detection notification sent to frontend for {name}")
                        except Exception as e:
                            logger.error(
                                f"Error sending drift detection notification: {e}")
                except Exception as e:
                    logger.exception(
                        f"Error storing drift results in Redis: {e!s}")
            ...
            logger.exception(f"Error detecting drift: {e!s}")
        ...
        try:
            ...
            if model_type == "classification":
                for metric in ["accuracy", "precision", "recall", "f1"]:
                    if metric in current_metrics and metric in baseline_metrics:
                        ...
                        pct_change = (
                            change / baseline_metrics[metric]
            ...
            if self.redis:
                try:
                    ...
                    logger.exception(
                        f"Error storing performance drift results in Redis: {e!s}",
                    )
            ...
            logger.exception(f"Error detecting performance drift: {e!s}")
            return {"drift_detected": False, "error": str(e)}
...
class ModelEvaluator:
    ...
    def evaluate_classification_model(
        self, model, model_type: str, X_test, y_test, feature_names: list[str] | None = None,
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if self.redis:
                try:
                    # Store main metrics (exclude large elements like
                    # feature_importance)
                    redis_metrics = {
                        k: v
                        for k, v in metrics.items()
                    ...
                    logger.exception(
                        f"Error storing evaluation metrics in Redis: {e!s}")

            return metrics

        except Exception as e:
            logger.exception(f"Error evaluating classification model: {e!s}")
            return {"error": str(e)}
    ...
    def evaluate_regression_model(
        ...
        try:
            ...
            if model_type == "xgboost":
                ...
                # Handle multi-dim output
                if len(y_pred.shape) > 1 and y_pred.shape[1] > 1:
                    msg = "Multi-output regression not supported for evaluation"
                    raise ValueError(
                        msg,
                    )
            elif model_type == "sklearn":
            ...
            logger.exception(f"Error evaluating regression model: {e!s}")
            return {"error": str(e)}

    def _get_feature_importance(
        self, model, model_type: str, feature_names: list[str] | None = None,
    ) -> dict[str, float]:
        ...
        try:
            ...
            if model_type == "xgboost":
                ...
                if feature_names:
                    importance = {
                        name: importance_map.get(name, 0) for name in feature_names
                    }
                else:
                ...
                if hasattr(model, "feature_importances_"):
                    ...
                    if feature_names and len(imp) == len(feature_names):
                        importance = {
                            name: float(imp[i]) for i, name in enumerate(feature_names)
                        }
                    else:
                        importance = {
                            f"feature_{i}": float(imp[i]) for i in range(len(imp))
                        }
            ...
            logger.exception(f"Error extracting feature importance: {e!s}")
            return {}
...
class LearningEngine:
    ...
    def __init__(
        self, redis_client=None, ml_engine=None, data_pipeline=None, use_gpu=True,
    ) -> None:
        """
        ...
        self.config = {
            ...
            "performance_threshold": float(
                os.environ.get("PERFORMANCE_THRESHOLD", "0.8"),
            ),
            "max_versions": int(os.environ.get("MAX_MODEL_VERSIONS", "5")),
            "drift_detection": {
                "enabled": os.environ.get("DRIFT_DETECTION_ENABLED", "true").lower()
                == "true",
                "auto_retrain": os.environ.get("DRIFT_AUTO_RETRAIN", "true").lower()
                == "true",
        ...
        self.model_registry = ModelRegistry(
            models_dir=self.config["models_dir"], redis_client=redis_client,
        )

        self.model_deployer = ModelDeployer(
            model_registry=self.model_registry, redis_client=redis_client,
        )

        self.performance_tracker = ModelPerformanceTracker(
            redis_client=redis_client)

        self.drift_detector = DriftDetector(redis_client=redis_client)

        self.model_evaluator = ModelEvaluator(redis_client=redis_client)
        ...
        logger.info(
            "Learning Engine initialized with GPU acceleration"
            if self.use_gpu
            else "Learning Engine initialized (CPU mode)",
        )

    def _send_frontend_notification(self, message, level="info", category="learning_engine", details=None):
        ...
        try:
            ...
            if category in ["system_status", "learning_system"]:
                try:
    ...
    def _log_gpu_memory(self) -> None:
        ...
        try:
            if CUPY_AVAILABLE and cp is not None:
                ...
                if self.redis:
                    self.redis.hset(
                        "system:gpu", "memory_used_mb", int(
                            used / (1024 * 1024)),
                    )
                    self.redis.hset(
                        "system:gpu", "memory_free_mb", int(
                            free / (1024 * 1024)),
                    )
                    self.redis.hset(
                        "system:gpu", "memory_total_mb", int(
                            total / (1024 * 1024)),
                    )
        except Exception as e:
            logger.exception(f"Error getting GPU memory info: {e}")

    def _clear_gpu_memory(self) -> bool | None:
        ...
        try:
            ...
            logger.exception(f"Error clearing GPU memory: {e}")
            return False
    ...
    def start(self) -> None:
        ...
        self._send_frontend_notification(
            message="Learning Engine starting",
            level="info",
            category="learning_system",
            details={
                "gpu_enabled": self.use_gpu,
                "device_name": self.accelerator.device_name if hasattr(self, "accelerator") and hasattr(self.accelerator, "device_name") else "CPU",
                "config": {
                    "training_schedule": self.config["training_schedule"],
                    "drift_detection": self.config["drift_detection"]["enabled"],
    ...
    def stop(self) -> None:
        ...
        # Send notification to frontend
        self._send_frontend_notification(
            message="Learning Engine shutting down",
            level="info",
            category="learning_system",
            details={
    ...
    def _scheduler_thread(self) -> None:
        ...
        while self.running:
            try:
                ...
                logger.error(
                    f"Error in scheduler thread: {e!s}", exc_info=True)
                time.sleep(60)  # Back off on error

    def _drift_detection_thread(self) -> None:
        ...
        while self.running:
            try:
                ...
                logger.error(
                    f"Error in drift detection thread: {e!s}", exc_info=True,
                )
                time.sleep(300)  # Back off on error

    def daily_model_update(self) -> bool | None:
        """Perform daily incremental update of models"""
        logger.info("Starting daily model update")

        # Send notification to frontend
        self._send_frontend_notification(
            message="Starting daily model update",
            level="info",
            category="model_training",
            details={
                "update_type": "daily",
                "update_window": self.config["update_window"],
                "start_time": time.time()
            }
        )
        ...
        try:
            ...
            for model_name, version in deployed_models.items():
                try:
                    ...
                    logger.error(
                        f"Error updating {model_name} model: {e!s}", exc_info=True,
                    )
            ...
            # Send notification to frontend about completed update
            self._send_frontend_notification(
                message=f"Daily model update completed: {len(updated_models)} models updated",
                level="success",
                category="model_training",
                details={
                    "update_type": "daily",
                    ...
                    "duration": time.time() - self.training_start_time if hasattr(self, "training_start_time") else None
                }
            )
            ...
            logger.error(f"Error in daily model update: {e!s}", exc_info=True)
            return False
    ...
    def full_model_retrain(self):
        ...
        # Send notification to frontend
        self._send_frontend_notification(
            message="Starting full model retraining",
            level="info",
            category="model_training",
            details={
        ...
        try:
            ...
            if self.ml_engine:
                ...
                if success:
                    ...
                    # Send notification to frontend
                    self._send_frontend_notification(
                        message="Full model retraining completed successfully",
                        level="success",
                        category="model_training",
                        details={
                            "update_type": "full_retrain",
                            "completion_time": time.time(),
                            "duration": time.time() - self.training_start_time if hasattr(self, "training_start_time") else None,
                            "models_registered": list(self.model_registry.get_all_models().keys())
                        }
                    )
            ...
            logger.error(
                f"Error in full model retraining: {e!s}", exc_info=True)
            return False

    def _register_ml_engine_models(self) -> bool | None:
        ...
        try:
            ...
            for model_name, model_type in model_types.items():
                ...
                try:
                    ...
                    logger.exception(
                        f"Error registering {model_name} model: {e!s}")

            return True

        except Exception as e:
            logger.exception(f"Error registering ML engine models: {e!s}")
            return False

    def _train_all_models_internally(self) -> bool:
    ...
    def _load_recent_data(self):
        ...
        try:
            ...
            start_date = end_date - datetime.timedelta(
                days=self.config["update_window"],
            )
            ...
            options_data = self.data_pipeline.load_options_data(
                tickers=active_tickers, start_date=start_date, end_date=end_date,
            )
            ...
            logger.error(f"Error loading recent data: {e!s}", exc_info=True)
            return None
    ...
    def _get_active_tickers(self):
        ...
        try:
            ...
            # Fallback to default tickers from configuration if ML engine is not
            # available
            if not self.ml_engine:
                default_tickers = config["stock_selection"]["universe"][
                    "default_tickers"
                ]
                logger.info(
            ...
            logger.exception(f"Error getting active tickers: {e!s}")
    ...
    def _prepare_training_data(self, price_data, options_data, market_data):
        ...
        try:
            ...
            logger.error(
                f"Error preparing training data: {e!s}", exc_info=True)
            return None
    ...
    def _prepare_model_specific_data(self, model_name, data):
        ...
        try:
            ...
            if isinstance(data, pd.DataFrame):
                ...
                if model_name in ["signal_detection", "risk_assessment"]:
                    ...
                    if "price_close" in data.columns and "price_open" in data.columns:
                        y = (
                            (data["price_close"] > data["price_open"])
                            .astype(int)
                            .values
                        )
                    else:
                ...
                if model_name in ["price_prediction", "exit_strategy"]:
                    numeric_cols = data.select_dtypes(
                        include=["float64", "int64"],
                    ).columns
                    X = data[numeric_cols].fillna(0).values
            ...
            logger.error(
                f"Error preparing model-specific data for {model_name}: {e!s}",
                exc_info=True,
            )
            return None, None
        ...
        try:
            ...
            if model_name == "signal_detection":
                ...
                metrics["precision"] = float(
                    precision_score(y_val, y_pred_binary, zero_division=0),
                )
                metrics["recall"] = float(
                    recall_score(y_val, y_pred_binary, zero_division=0),
                )
                metrics["f1"] = float(
            ...
            if hasattr(updated_model, "eval_result"):
                ...
                for metric in updated_model.eval_result:
                    for dataset in updated_model.eval_result[metric]:
            ...
            logger.error(
                f"Error updating XGBoost model {model_name}: {e!s}", exc_info=True,
            )
    ...
    def _update_keras_model(self, model_name, model, features, target):
        ...
        try:
            ...
            model.compile(
                ...
                loss=(
                    "binary_crossentropy" if model_name == "signal_detection" else "mse"
                ),
                metrics=(
                    ["accuracy", "AUC"]
                    if model_name == "signal_detection"
            ...
            if logger.level <= logging.DEBUG:
                ...
                tensorboard_callback = tf.keras.callbacks.TensorBoard(
                    ...
                    histogram_freq=1,
                    profile_batch=(
                        "500,520" if self.use_gpu else 0
                    ),  # Only profile on GPU
                )
            ...
            if self.use_gpu and TENSORRT_AVAILABLE and trt is not None:
                try:
                    ...
                    metrics["tensorrt_precision"] = self.config["gpu"][
                        "tensorrt_precision"
                    ]
                    ...
                    # We don't return the TensorRT model directly as it's not compatible
            ...
            logger.error(
                f"Error updating Keras model {model_name}: {e!s}", exc_info=True,
            )
        ...
        try:
            ...
            if hasattr(model, "partial_fit"):
                ...
                if model_name in ["signal_detection", "risk_assessment"]:
                    ...
                    metrics["recall"] = float(
                        recall_score(target, y_pred, zero_division=0),
                    )
                    metrics["f1"] = float(
            ...
            if model_name == "market_regime":
                ...
                if isinstance(model, KMeans):
                    ...
                    # Create a new model with the same parameters but initialized with
                    # previous centroids
                    new_model = KMeans(
            ...
            logger.error(
                f"Error updating scikit-learn model {model_name}: {e!s}",
                exc_info=True,
            )
            return False, None, {}

    def _should_deploy_model(self, model_name, version, metrics) -> bool | None:
        ...
        try:
            ...
            try:
                ...
                logger.exception(f"Error getting current model metrics: {e!s}")
            ...
            logger.exception(
                f"Error in deployment decision for {model_name} v{version}: {e!s}",
            )
    ...
    def check_for_drift(self):
        ...
        # Send notification to frontend
        self._send_frontend_notification(
            message="Starting drift detection check",
            level="info",
            category="drift_detection",
            details={
                "check_time": time.time(),
                "drift_detection_enabled": self.config["drift_detection"]["enabled"],
                "auto_retrain": self.config["drift_detection"]["auto_retrain"]
            }
        )
        ...
        try:
            ...
            if any_drift:
                # Send notification to frontend about drift detection
                self._send_frontend_notification(
                    message="Data or model drift detected",
                    level="warning",
                    category="drift_detection",
                    details={
                ...
                if self.config["drift_detection"]["auto_retrain"]:
                    logger.info(
                        "Drift detected, triggering full model retraining")
                    self.full_model_retrain()
            else:
                # Send notification to frontend about completed check with no drift
                self._send_frontend_notification(
                    message="Drift detection check completed - no drift detected",
                    level="success",
                    category="drift_detection",
                    details={
                        "check_time": time.time(),
                        "models_checked": list(self.model_deployer.get_all_deployed_models().keys())
                    }
                )
            ...
            logger.error(
                f"Error in scheduled drift check: {e!s}", exc_info=True)
            return False
        ...
        try:
            ...
            for model_name in deployed_models:
                ...
                if drift_results.get("drift_detected", False):
                    ...
                    drifted_features = [
                        f
                        for f, v in drift_features.items()
                        if isinstance(v, dict) and v.get("drift_detected", False)
                    ]
            ...
            logger.error(f"Error checking data drift: {e!s}", exc_info=True)
            return False
        ...
        try:
            ...
            for model_name, version in deployed_models.items():
                ...
                try:
                    ...
                    logger.exception(
                        f"Error getting current model metrics: {e!s}")
            ...
            logger.error(
                f"Error checking performance drift: {e!s}", exc_info=True)
            return False

    def _check_for_data_drift(self) -> None:
        ...
        try:
            ...
            logger.exception(f"Error in periodic drift check: {e!s}")
    ...
    def _detect_market_regime_change(self, market_data):
        ...
        try:
            ...
            logger.exception(f"Error detecting market regime change: {e!s}")
            return None

    def create_model_challenger(self, model_name, hyperparams=None) -> bool | None:
        ...
        try:
            ...
            if hyperparams is None:
                ...
                if model_type == "xgboost":
                    hyperparams = {
                        "max_depth": (
                            7
                            if metadata.get("hyperparams", {}).get("max_depth", 6) == 6
                            else 6
                        ),
                        "eta": (
                            0.05
                            if metadata.get("hyperparams", {}).get("eta", 0.1) == 0.1
                            else 0.1
                        ),
                        "subsample": (
                            0.9
                            if metadata.get("hyperparams", {}).get("subsample", 0.8)
                            == 0.8
                            else 0.8
                        ),
                        "objective": (
            ...
            if model_type == "xgboost":
                ...
                if model_name == "signal_detection":
                    ...
                    metrics["precision"] = float(
                        precision_score(target, y_pred_binary,
                                        zero_division=0),
                    )
                    metrics["recall"] = float(
                        recall_score(target, y_pred_binary, zero_division=0),
                    )
                    metrics["f1"] = float(
                ...
                challenger_model = metadata.get("model_class", RandomForestClassifier)(
                    **hyperparams,
                )
            ...
            if challenger_model is not None:
                ...
                return True
            logger.error(f"Failed to create challenger model for {model_name}")
            return False

        except Exception as e:
            logger.error(
                f"Error creating challenger model for {model_name}: {e!s}",
                exc_info=True,
            )
            return False
...
class ContinualLearningSystem:
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            try:
                ...
                logging.getLogger("continual_learning").warning(
                    f"Could not start Prometheus server: {e!s}",
                )
    ...
    def daily_model_update(self):
        ...
        if PROMETHEUS_AVAILABLE and success:
            for model_name in get_active_models():
                # Get current version
                current_version = (
                    self.learning_engine.model_deployer.deployed_models.get(
                        model_name)
                )
                if current_version:
                    # Get metrics
                    ...
                    if metrics_data:
                        ...
                        if "accuracy" in metrics_data:
                            self.metrics["model_accuracy"].labels(
                                model_name=model_name, version=current_version,
        ...
        if PROMETHEUS_AVAILABLE and success:
            for model_name in get_active_models():
                self.metrics["training_duration"].labels(
                    model_name=model_name, version="full_retrain",
                ).observe(duration)

                self.metrics["retraining_count"].labels(
                    model_name=model_name, reason="scheduled",
                ).inc()
    ...
    def check_for_drift(self):
        ...
        if PROMETHEUS_AVAILABLE and drift_detected:
            for model_name in get_active_models():
                self.metrics["drift_score"].labels(data_type=model_name).set(
                    1.0,
                ...
                if self.learning_engine.config["drift_detection"]["auto_retrain"]:
                    self.metrics["retraining_count"].labels(
                        model_name=model_name, reason="drift",
                    ).inc()
