...
def check_redis():
    ...
    try:
        ...
        try:
            ...
            error_msg = f"Failed to start Redis: {e!s}"
    ...
    try:
        ...
        try:
            ...
            if os.path.exists(prometheus_conf):
                subprocess.Popen(
                    ["prometheus", "--config.file=" + prometheus_conf],
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
                )
            else:
            ...
            error_msg = f"Failed to start Prometheus: {e!s}"
            logger.exception(error_msg)
...
def start_monitoring_system(redis_client=None):
    ...
    try:
        ...
        error_msg = f"Failed to start monitoring system: {e!s}"
        logger.exception(error_msg)
...
def start_individual_components(redis_client=None, use_gpu=True) -> bool | None:
    ...
    try:
        ...
        error_msg = f"Failed to start individual components: {e!s}"
        logger.exception(error_msg)
...
def stop_all_components() -> None:
    ...
    if monitoring_system and hasattr(monitoring_system, "stop"):
        try:
            ...
            if slack_notifier:
                slack_notifier.send_warning(
                    f"Error stopping monitoring system: {e!s}",
                )
...
def run_health_check(redis_client=None):
    ...
    health_status = {
        ...
        "components": {},
        "services": {"redis": False, "prometheus": False},
        "gpu": {},
    }
    ...
    if monitoring_system:
        health_status["components"]["monitoring_system"] = {
            "running": hasattr(monitoring_system, "running")
            and monitoring_system.running,
            "status": (
                "healthy"
                if hasattr(monitoring_system, "running") and monitoring_system.running
                else "not running"
            ),
        }
    ...
    if slack_notifier:
        ...
        components_healthy = all(
            c.get("running", False) for c in health_status["components"].values()
        )
        services_healthy = all(health_status["services"].values())
        system_healthy = (
            health_status["system"]["cpu_usage_percent"] < 90
            and health_status["system"]["memory_usage_percent"] < 90
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--no-redis", action="store_true", help="Skip Redis check/start",
    )
    parser.add_argument(
        "--no-prometheus", action="store_true", help="Skip Prometheus check/start",
    )
    parser.add_argument(
        "--individual",
        action="store_true",
...
def main() -> None:
        ...
        if not redis_client:
            ...
            if slack_notifier:
                slack_notifier.send_error(
                    ...
                    "Redis is required but could not be started",
                )
            sys.exit(1)
    ...
    try:
        if args.individual:
            ...
            if start_individual_components(
                redis_client=redis_client, use_gpu=not args.no_gpu,
            ):
            ...
            if system.start():
                ...
                if slack_notifier:
                    slack_notifier.send_error(
                        "System startup failed",
                        "Failed to start the unified trading system",
                    )
                sys.exit(1)

        # Run initial health check
        run_health_check(redis_client)
        ...
        error_msg = f"Error starting trading system: {e!s}"
        logger.exception(error_msg)
        if slack_notifier:
