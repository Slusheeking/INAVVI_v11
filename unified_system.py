...
class UnifiedTradingSystem:
    ...
    def __init__(self, config_path=None, debug=False, use_gpu=True) -> None:
        """
    ...
    def _load_config(self, config_path):
        ...
        if config_path and os.path.exists(config_path):
            try:
                ...
                logger.exception(
                    f"Error loading config from {config_path}: {e!s}")
    ...
    def _merge_configs(self, base, override) -> None:
        ...
        try:
            ...
            try:
                ...
                if physical_devices:
                    ...
                    # Configure memory growth to prevent TensorFlow from allocating all
                    # GPU memory
            ...
            if not hasattr(accelerator, "has_tensorflow_gpu"):
                accelerator.has_tensorflow_gpu = (
                    has_tensorflow and bool(physical_devices)
                    if "physical_devices" in locals()
                    else False
                )
            ...
            if has_tensorflow:
                accelerator.tensorflow_version = (
                    tf_version if "tf_version" in locals() else "Unknown"
                )

            # Add CuPy version info
            if has_cupy:
                accelerator.cupy_version = (
                    cupy_version if "cupy_version" in locals() else "Unknown"
                )
            ...
            if mem_info:
                ...
                if self.redis:
                    ...
                    self.redis.hset(
                        "system:gpu",
                        "tensorflow_gpu",
                        str(accelerator.has_tensorflow_gpu),
                    )
                    self.redis.hset(
                        "system:gpu", "cupy_gpu", str(
                            accelerator.has_cupy_gpu),
                    )
                    self.redis.hset(
                        "system:gpu", "tensorrt", str(
                            accelerator.has_tensorrt),
                    )
                    self.redis.hset(
                        "system:gpu",
                        "device_name",
                    ...
                    if has_tensorflow:
                        self.redis.hset(
                            ...
                            (
                                accelerator.tensorflow_version
                                if hasattr(accelerator, "tensorflow_version")
                                else "Unknown"
                            ),
                        )
                    if has_tensorrt:
                        self.redis.hset(
                            ...
                            "tensorrt_version",
                            (
                                accelerator.tensorrt_config["version"]
                                if hasattr(accelerator, "tensorrt_config")
                                else "Unknown"
                            ),
                        )
                    if has_cupy:
                        self.redis.hset(
                            ...
                            (
                                accelerator.cupy_version
                                if hasattr(accelerator, "cupy_version")
                                else "Unknown"
                            ),
                        )
            ...
            logger.exception(f"Error initializing GPU: {e!s}")
            return None
    ...
    def _initialize_redis(self):
        ...
        try:
            ...
            # Initialize frontend status in Redis
            if redis_client:
                self._update_frontend_status(redis_client, "initializing")

            return redis_client
        except Exception as e:
            logger.exception(f"Error connecting to Redis: {e!s}")
            logger.exception(
    ...
    def _update_frontend_status(self, redis_client, status, details=None):
        """Update frontend status in Redis"""
        if not redis_client:
            return
        ...
        try:
            ...
            redis_client.hset("frontend:status", mapping=status_data)

            # Store in history (keep last 100 status updates)
            redis_client.lpush("frontend:status:history",
                               json.dumps(status_data))
            redis_client.ltrim("frontend:status:history", 0, 99)

            # Publish real-time status update for frontend
            redis_client.publish("frontend:events", json.dumps({
                "type": "system_status_update",
                "data": status_data
            }))

            logger.debug(f"Frontend status updated: {status}")
        except Exception as e:
            logger.exception(f"Error updating frontend status: {e}")
    ...
    async def _initialize_async_components(self) -> bool | None:
        """Initialize async components"""
        try:
            # Data pipeline - handles all market data streams
            logger.info("Initializing data pipeline...")
            data_pipeline = DataPipeline(
                redis_client=self.redis, use_gpu=self.use_gpu)
            ...
            logger.exception(f"Error initializing async components: {e!s}")
            return False

    def _initialize_components(self) -> bool | None:
        ...
        try:
            ...
            # The trading_engine component initialized earlier handles all trading and
            # execution logic

            # Monitoring system - metrics and alerting
            logger.info("Initializing monitoring system...")
            monitoring_system = MonitoringSystem(
                redis_client=self.redis, config=self.config,
            )
            ...
            logger.exception(f"Error initializing components: {e!s}")
            return False

    async def _start_async_components(self) -> bool | None:
        """Start asynchronous components"""
        try:
            for name, component in self.components.items():
                if hasattr(component, "start") and asyncio.iscoroutinefunction(
                    component.start,
                ):
            ...
            logger.exception(f"Error starting async components: {e!s}")
            return False

    def _start_components(self) -> bool | None:
        """Start synchronous components"""
        try:
            for name, component in self.components.items():
                if hasattr(component, "start") and not asyncio.iscoroutinefunction(
                    component.start,
                ):
            ...
            logger.exception(f"Error starting components: {e!s}")
            return False

    async def _stop_async_components(self) -> None:
        """Stop asynchronous components"""
        for name, component in reversed(list(self.components.items())):
            if hasattr(component, "stop") and asyncio.iscoroutinefunction(
                component.stop,
            ):
    ...
    def _stop_components(self) -> None:
        ...
        for name, component in reversed(list(self.components.items())):
            if hasattr(component, "stop") and not asyncio.iscoroutinefunction(
                component.stop,
            ):
        ...
        while self.running:
            try:
                ...
                if self.use_gpu and self.gpu_accelerator:
                    try:
                        from gpu_utils import clear_gpu_memory, get_gpu_memory_usage
                        ...
                        if mem_info:
                            ...
                            if self.redis:
                                ...
                                self.redis.lpush(
                                    "system:gpu_memory_history", json.dumps(
                                        memory_data),
                                )
                                self.redis.ltrim(
                                    "system:gpu_memory_history", 0, 99)

                                # Publish real-time GPU memory update for frontend
                                ...
                                if (
                                    hasattr(self.gpu_accelerator,
                                            "has_tensorflow_gpu")
                                    and self.gpu_accelerator.has_tensorflow_gpu
                                ):
                                    try:
                                        import tensorflow as tf
                                        ...
                                        if tf_mem_info:
                                            self.redis.hset(
                                                "system:metrics",
                                                "tensorflow_memory_info",
                                                json.dumps(tf_mem_info),
                                            )
                                    except Exception as e:
                                        logger.debug(
                                ...
                                if (
                                    hasattr(self.gpu_accelerator,
                                            "has_cupy_gpu")
                                    and self.gpu_accelerator.has_cupy_gpu
                                ):
                                    try:
                                        import cupy as cp
                            ...
                            if usage_pct > 95.0:
                                ...
                                # Publish GPU memory critical alert for frontend
                                self.redis.publish("frontend:events", json.dumps({
                                ...
                                if (
                                    ...
                                    try:
                                        ...
                                        # Reset TensorFlow GPU memory stats for each
                                        # device
                                        for device in tf.config.list_physical_devices(
                                            "GPU",
                                        ):
                                ...
                                # If we have an accelerator with TensorFlow, clear its
                                # session too
                                ...
                                for i in range(
                                    3,
                                ):  # Run multiple cycles for better cleanup
                                    gc.collect(i)
                                ...
                                # Publish GPU memory warning alert for frontend
                                self.redis.publish("frontend:events", json.dumps({
                                ...
                                scheduled_cleanup_interval = int(
                                    os.environ.get(
                                        "GPU_MEMORY_CLEANUP_INTERVAL", 300),
                                )
                        ...
                        logger.exception(f"Error monitoring GPU memory: {e!s}")
                ...
                for name, component in self.components.items():
                    if hasattr(component, "running"):
                        if not component.running:
                            ...
                            # Publish component restart event for frontend
                            if self.redis:
                                self.redis.publish("frontend:events", json.dumps({
                                    ...
                                    "data": {
                                        "component": name,
                                        "timestamp": time.time(),
                                        "message": f"Attempting to restart {name} component"
                                    }
                                }))
                ...
                # Update frontend status with system health information
                if self.redis:
                    system_health = {
                        "components": {},
                    ...
                    for name, component in self.components.items():
                        if hasattr(component, "running"):
                            system_health["components"][name] = "running" if component.running else "stopped"
                        else:
                            system_health["components"][name] = "unknown"
                    ...
                    # Publish real-time component status update for frontend
                    self.redis.publish("frontend:events", json.dumps({
                        "type": "component_status_update",
                        "data": {
                            "components": system_health["components"],
                            "timestamp": system_health["last_update"]
                        }
                    }))

                    # Update frontend status
                    self._update_frontend_status(
                        self.redis, "running", system_health)
                ...
                logger.exception(f"Error in main loop: {e!s}")
    ...
    async def _run_async(self) -> None:
        ...
        try:
            ...
            logger.exception(f"Error in async operation: {e!s}")
        finally:
            self.running = False
    ...
    def _run_event_loop(self) -> None:
        ...
        try:
            ...
            logger.exception(f"Error in event loop thread: {e!s}")
        finally:
    ...
    def start(self) -> bool:
        ...
        # Update frontend status
        if self.redis:
            self._update_frontend_status(self.redis, "starting")

            # Publish system startup event for real-time frontend updates
            self.redis.publish("frontend:events", json.dumps({
                "type": "system_startup",
                "data": {
                    "timestamp": time.time(),
        ...
        if not self._initialize_components():
            ...
            if self.redis:
                self._update_frontend_status(
                    self.redis, "error", {"error": "Failed to initialize components"})
            return False
        ...
        if not self._start_components():
            ...
            if self.redis:
                self._update_frontend_status(
                    self.redis, "error", {"error": "Failed to start components"})
            return False
        ...
        # Update frontend status
        if self.redis:
            self._update_frontend_status(self.redis, "running")
    ...
    def stop(self) -> bool:
        ...
        # Update frontend status
        if self.redis:
            self._update_frontend_status(self.redis, "stopping")
        ...
        # Update frontend status
        if self.redis:
            self._update_frontend_status(self.redis, "stopped")

            # Publish final system shutdown event for real-time frontend updates
            self.redis.publish("frontend:events", json.dumps({
                "type": "system_shutdown_complete",
                "data": {
                    "timestamp": time.time(),
                    "message": "System shutdown complete",
                    "uptime_seconds": int(time.time() - self.redis.hget("frontend:status", "timestamp") or 0)
                }
            }))
        ...
        if self.use_gpu and self.gpu_accelerator:
            ...
            try:
                ...
                # TensorFlow cleanup
                if (
                    hasattr(self.gpu_accelerator, "has_tensorflow_gpu")
                    and self.gpu_accelerator.has_tensorflow_gpu
                ):
                    try:
                        import tensorflow as tf
                ...
                # TensorRT cleanup
                if (
                    hasattr(self.gpu_accelerator, "has_tensorrt")
                    and self.gpu_accelerator.has_tensorrt
                ):
                    try:
                ...
                # CuPy cleanup
                if (
                    hasattr(self.gpu_accelerator, "has_cupy_gpu")
                    and self.gpu_accelerator.has_cupy_gpu
                ):
                    try:
                        import cupy as cp
                ...
                try:
                    ...
                    if mem_info and self.redis:
                        ...
                        self.redis.hset(
                            "system:shutdown", "gpu_memory_used_mb", int(
                                used_mb),
                        )
                        self.redis.hset(
                            "system:shutdown", "gpu_memory_free_mb", int(
                                free_mb),
                        )
                        self.redis.hset(
                            "system:shutdown", "gpu_memory_total_mb", int(
                                total_mb),
                        )
                        self.redis.hset(
                            "system:shutdown", "gpu_memory_pct", float(
                                usage_pct),
                        )
                        self.redis.hset(
                            "system:shutdown", "timestamp", int(time.time()),
                        )

                        # Store framework-specific cleanup status
                        cleanup_status = {
                            "tensorflow": hasattr(
                                self.gpu_accelerator, "has_tensorflow_gpu",
                            )
                            and self.gpu_accelerator.has_tensorflow_gpu,
                            "tensorrt": hasattr(self.gpu_accelerator, "has_tensorrt")
                            and self.gpu_accelerator.has_tensorrt,
                            "cupy": hasattr(self.gpu_accelerator, "has_cupy_gpu")
                            and self.gpu_accelerator.has_cupy_gpu,
                            "timestamp": int(time.time()),
                        }
                        self.redis.hset(
                ...
                logger.exception(f"Error cleaning up GPU resources: {e}")
    ...
    def status(self):
        ...
        for name, component in self.components.items():
            if hasattr(component, "running"):
                status["components"][name] = (
                    "running" if component.running else "stopped"
                )
            else:
                status["components"][name] = "unknown"

        # Frontend status
        if self.redis and self.redis.exists("frontend:status"):
            try:
                frontend_status = self.redis.hgetall("frontend:status")
                status["frontend"] = frontend_status
            except Exception as e:
                logger.exception(f"Error getting frontend status: {e}")
                status["frontend"] = {"status": "unknown", "error": str(e)}
        ...
        if self.use_gpu and self.gpu_accelerator:
            ...
            # Framework support
            status["gpu"]["tensorflow"] = (
                "enabled"
                if hasattr(self.gpu_accelerator, "has_tensorflow_gpu")
                and self.gpu_accelerator.has_tensorflow_gpu
                else "disabled"
            )
            status["gpu"]["cupy"] = (
                "enabled"
                if hasattr(self.gpu_accelerator, "has_cupy_gpu")
                and self.gpu_accelerator.has_cupy_gpu
                else "disabled"
            )
            status["gpu"]["tensorrt"] = (
                "enabled"
                if hasattr(self.gpu_accelerator, "has_tensorrt")
                and self.gpu_accelerator.has_tensorrt
                else "disabled"
            )
            ...
            # Device information
            if (
                hasattr(self.gpu_accelerator, "device_name")
                and self.gpu_accelerator.device_name
            ):
            ...
            if self.redis and self.redis.exists("system:gpu_memory_history"):
                try:
                    ...
                    logger.exception(f"Error getting GPU memory history: {e}")
            ...
            if self.redis and self.redis.exists("system:last_gpu_cleanup"):
                try:
                    ...
                    logger.exception(
                        f"Error getting last GPU cleanup time: {e}")
        else:
            status["gpu"]["acceleration"] = "disabled"
            status["gpu"]["reason"] = (
                "GPU acceleration was disabled by configuration"
                if not self.use_gpu
                else "No compatible GPU accelerator was found"
            )
...
def main() -> None:
    ...
    system = UnifiedTradingSystem(
        config_path=args.config, debug=args.debug, use_gpu=not args.no_gpu,
    )
...
UnifiedSystem = UnifiedTradingSystem
